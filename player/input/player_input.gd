...
func left_input() -> void:
	...
	knockback.apply(hand.get_child(1).on_left_input(camera_ray), camera_ray.global_position.direction_to(camera_point.global_position))

func right_input() -> void:
	if !_holding_something():
		return #do basic hand stuff here
	knockback.apply(hand.get_child(1).on_right_input(camera_ray),  camera_ray.global_position.direction_to(camera_point.global_position))
